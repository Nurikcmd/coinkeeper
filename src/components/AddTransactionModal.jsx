...
function AddTransactionModal({ isOpen, onClose }) {
  ...
  useEffect(() => {
    console.log('Текущие счета:', accounts)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert('Ошибка при добавлении транзакции: ' + (error.response?.data?.message || error.message))
    }
  }
            ...
            >
              ...
              {Array.isArray(accounts) && accounts.length > 0 ? (
                ...
                <option value="" disabled>
                  {accountsLoading ? 'Загрузка счетов...' : 'Нет доступных счетов'}
                </option>
              )}
            </select>
